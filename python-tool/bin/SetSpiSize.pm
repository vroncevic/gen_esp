package SetSpiSize;
#
# @brief    Set spi size for project configuration
# @version  ver.1.0
...
# ...
#
# my $status = setspisize(\%argStructure, \$option);
#
# if ($status == $SUCCESS) {
#	# true
#	# notify admin | user
...
sub setspisize {
	...
	if(%argStructure) {
		...
		my $xmlBoot = "$argStructure{PREFERENCES}->{SET_PRO_SPI_SIZE}";
		...
		foreach my $key (keys(%{$xmlBootOptions->{firmware}})) {
			my $spiSizeOption = $xmlBootOptions->{firmware}{$key}{option};
			my $spiSize = $xmlBootOptions->{firmware}{$key}{size};
			if($spiSizeOption eq "0") {
				$spiSizeOption = "$spiSizeOption (default)";
			}
			$table->add(
				$spiSizeOption,
				$spiSize
...
SetSpiSize - Set spi size for project configuration
...
Set spi size for project configuration

=head2 EXPORT

setspisize - return 0 for success, else return 1
