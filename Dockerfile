...
COPY pyproject.toml /
RUN mkdir /tests/
RUN find /gen_esp/ -name "*.editorconfig" -type f -exec rm -Rf {} \;
RUN python2 -m build --no-isolation --wheel
RUN pip2 install ./dist/gen_esp-*-py2-none-any.whl
RUN python3 -m build --no-isolation --wheel
RUN pip3 install ./dist/gen_esp-*-py3-none-any.whl
RUN rm -rf /gen_esp*
RUN rm -f setup.py
RUN rm -f README.md
RUN rm -f LICENSE
