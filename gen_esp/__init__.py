...
'''
 ...
 Info
     Defined class GenESP with attribute(s) and method(s).
...
__copyright__ = 'Copyright 2022, https://vroncevic.github.io/GenESP'
...
class GenESP(CfgCLI):
    '''
        ...
        It defines:
            ...
            :methods:
                ...
                | __str__ - dunder method for GenESP.
    '''

    __metaclass__ = CooperativeMeta
    GEN_VERBOSE = 'GEN_ESP'
    CONFIG = '/conf/gen_esp.cfg'
    LOG = '/log/gen_esp.log'
    LOGO = '/conf/gen_esp.logo'
    ...
    def __init__(self, verbose=False):
        ...
        gen_esp_property = {
            ...
            'ats_repository': 'gen_esp',
            'ats_name': 'gen_esp',
            'ats_logo_path': '{0}{1}'.format(current_dir, GenESP.LOGO),
            'ats_use_github_infrastructure': True
        }
        splash = Splash(gen_esp_property, verbose=verbose)
        base_info = '{0}{1}'.format(current_dir, GenESP.CONFIG)
        CfgCLI.__init__(self, base_info, verbose=verbose)
        verbose_message(GenESP.GEN_VERBOSE, verbose, 'init tool info')
        self.logger = ATSLogger(
            GenESP.GEN_VERBOSE.lower(),
            '{0}{1}'.format(current_dir, GenESP.LOG),
            verbose=verbose
        )
        if self.tool_operational:
            self.add_new_option(
                GenESP.OPS[0], GenESP.OPS[1],
                dest='gen', help='generate option'
            )
            self.add_new_option(
                GenESP.OPS[2], GenESP.OPS[3],
                action='store_true', default=False,
                help='activate verbose mode for generation'
            )
            self.add_new_option(
                GenESP.OPS[4], action='version', version=__version__
            )
    ...
    def process(self, verbose=False):
        ...
        if self.tool_operational:
            ...
            if num_of_args_sys > 1:
                ...
                if operation not in GenESP.OPS:
                    sys.argv.append('-h')
            else:
                sys.argv.append('-h')
            ...
            if not pro_exists:
                if bool(getattr(args, 'gen')):
                    print(
                        '{0} {1} [{2}]'.format(
                            '[{0}]'.format(GenESP.GEN_VERBOSE.lower()),
                            'generating', getattr(args, 'gen')
                        )
                    )
                    generator = GenESPPro(verbose=verbose)
                    status = generator.gen_setup(
                        getattr(args, 'gen'), verbose
                    )
                    if status:
                        success_message(GenESP.GEN_VERBOSE, 'done\n')
                        ...
                        error_message(
                            GenESP.GEN_VERBOSE, 'generation failed'
                        )
                        self.logger.write_log(
                    ...
                    error_message(
                        GenESP.GEN_VERBOSE, 'provide project name'
                    )
                    self.logger.write_log(
                ...
                error_message(
                    GenESP.GEN_VERBOSE, 'project already exist'
                )
                self.logger.write_log(
            ...
            error_message(
                GenESP.GEN_VERBOSE, 'tool is not operational'
            )
            self.logger.write_log(
